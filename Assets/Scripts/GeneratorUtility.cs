...
public static class GeneratorUtility
{
    ...
    private static readonly int[] grad3 = new int[48] {
    ...
    private static float FastFloor(float x) { return x > 0 ? (int)x : (int)x - 1; }
    private static float Dot(int[] g, int index, float x, float y, float z) { return g[index] * x + g[index + 1] * y + g[index + 2] * z; }

    public static float EvaluateNoise(float x, float y, float z)
    {
        float n0, n1, n2, n3;
        float F3 = 1.0f / 3.0f;
        float s = (x + y + z) * F3;
    ...
    public static float FBM(Vector3 pos, float scale, float offset, int octaves)
    {
        ...
        {
            total += EvaluateNoise(pos.x * frequency + offset, pos.y * frequency + offset, pos.z * frequency + offset) * amplitude;
            maxValue += amplitude;
            amplitude *= 0.5f;
            frequency *= 2f;
        }
    ...
    public static float RidgedNoise(Vector3 pos, float scale, float offset, int octaves)
    {
        ...
        for (int i = 0; i < octaves; i++)
        {
            float n = EvaluateNoise(pos.x * frequency + offset, pos.y * frequency + offset, pos.z * frequency + offset);
            n = 1f - Mathf.Abs(n);
            n *= n;
            n *= weight;
